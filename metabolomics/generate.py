...
Example: m/z 427.3778 vs C26H50O4+H theoretical 427.3782
    ...
    <script>
        ...
        function renderFilteredPeaks(selector, data) {{
            ...
            headerRow.append('div').style('min-width', '25px').style('flex-shrink', '0').style('text-align', 'center').text('#');
            headerRow.append('div').style('min-width', '120px').style('flex-shrink', '0').text('Compound');
            headerRow.append('div').style('min-width', '90px').style('flex-shrink', '0').text('Formula');
            headerRow.append('div').style('min-width', '70px').style('flex-shrink', '0').style('text-align', 'right').text('m/z');
            headerRow.append('div').style('min-width', '35px').style('flex-shrink', '0').style('text-align', 'right').text('RT');
            headerRow.append('div').style('min-width', '40px').style('flex-shrink', '0').style('text-align', 'right').text('CV%');
            ...
            filtered.slice(0, 100).forEach(function(d, i) {{
                ...
                row.append('div').style('min-width', '25px').style('flex-shrink', '0').style('text-align', 'center').style('color', '#999').text(i + 1);

                var name = d.compound.length > 16 ? d.compound.substring(0, 13) + '...' : d.compound;
                row.append('div').style('min-width', '120px').style('flex-shrink', '0').style('font-family', 'monospace').style('font-size', '0.95em').style('overflow', 'hidden').style('text-overflow', 'ellipsis').style('white-space', 'nowrap').attr('title', d.compound).text(name);
                row.append('div').style('min-width', '90px').style('flex-shrink', '0').style('font-family', 'monospace').style('font-size', '0.95em').style('overflow', 'hidden').style('text-overflow', 'ellipsis').style('white-space', 'nowrap').style('color', d.formula ? '#2563eb' : '#ccc').text(d.formula || '-');
                row.append('div').style('min-width', '70px').style('flex-shrink', '0').style('text-align', 'right').style('font-family', 'monospace').text(d.mz ? parseFloat(d.mz).toFixed(4) : '-');
                row.append('div').style('min-width', '35px').style('flex-shrink', '0').style('text-align', 'right').style('font-family', 'monospace').text(d.rt ? parseFloat(d.rt).toFixed(1) : '-');

                // CV% with color coding
                var cvText = d.cv ? d.cv.toFixed(0) + '%' : '-';
                var cvColor = d.cv ? (d.cv < 30 ? '#16a34a' : d.cv < 60 ? '#ca8a04' : '#dc2626') : '#999';
                row.append('div').style('min-width', '40px').style('flex-shrink', '0').style('text-align', 'right').style('color', cvColor).text(cvText);
