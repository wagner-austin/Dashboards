...
export function stepCompanion(state: CompanionState, input: CompanionInput): CompanionState {
...
export function companionFrame(state: CompanionState, frames: BunnyFrames, sprint: SprintFrames): string[] {
  ...
  if (frame === undefined) throw new Error("RABBIT_COMPANION_FRAME: Animation has no frame at the requested index.");
  return frame.split("\n");
}

function transitionMode(state: CompanionState, requested: CompanionState["mode"]): CompanionState["mode"] {
  if (requested === "jump") return requested;
  if ((state.mode === "turnAway" || state.mode === "turnToward" || state.mode === "settle") && state.clock < 0.24) return state.mode;
  if (requested === "hopAway" && state.mode !== "hopAway" && state.mode !== "turnAway") return "turnAway";
  if (requested === "hopToward" && state.mode !== "hopToward" && state.mode !== "turnToward") return "turnToward";
  if (requested === "idle" && (state.mode === "walk" || state.mode === "run")) return "settle";
  return requested;
}

function selectSequence(state: CompanionState, frames: BunnyFrames, sprint: SprintFrames): readonly string[] {
