...
export const _test_hooks = {
  importModule: (url: string): Promise<unknown> => import(/* @vite-ignore */ url),
  fetchFn: (url: string): Promise<Response> => fetch(url),
 ...
 */
export async function importSpriteModule(path: string): Promise<SpriteModule> {
  ...
  const promise = (async (): Promise<SpriteModule> => {
    const module: unknown = await _test_hooks.importModule(assetUrl(path));
