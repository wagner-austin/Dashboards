...
beforeEach(() => {
  ...
  transportHooks.importModule = (url: string): Promise<unknown> =>
    Promise.resolve({ frames: [`frames:${new URL(url).pathname}`] });
});
...
describe("runProgressiveLoad", () => {
  ...
  it("reports the character it was asked for, not a hardcoded name", async () => {
    ...
    await runProgressiveLoad(
      createTestConfig(["lion"]),
      "lion",
      registry,
      (p) => {
        if (p.phase === "bunny") names.push(p.spriteName);
      },
      () => {
