...
describe("adventure asset loading", () => {
  it("loads both characters through the real shared transport and keeps run separate", async () => {
    transport.importModule = (url: string): Promise<unknown> => Promise.resolve({ frames: [new URL(url).pathname] });
  ...
  it("requires every part of the upright animation configuration", async () => {
    const base = config();
    const documents: readonly Config[] = [
      { ...base, sprites: {} },
      { ...base, sprites: { bunny: {} } },
      { ...base, sprites: { bunny: { animations: {} } } },
      { ...base, sprites: { bunny: { animations: { alert: { ...createTestAnimation(true, 40), widths: [] } } } } },
    ];
    for (const document of documents) await expect(loadAdventureAssets(document, "bunny")).rejects.toThrow("RABBIT_ALERT_CONFIG");
  });
  it("requires every part of the sprint animation configuration", async () => {
    ...
    for (const document of documents) await expect(loadAdventureAssets(document, "lion")).rejects.toThrow("RABBIT_RUN_CONFIG");
  });
});
