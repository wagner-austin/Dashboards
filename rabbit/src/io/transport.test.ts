...
beforeEach(() => {
  ...
  _test_hooks.importModule = (url: string): Promise<unknown> => {
    requested.push(url);
...
describe("loadSpriteFrames", () => {
  ...
  it("rejects a module with no frames array", async () => {
    _test_hooks.importModule = (): Promise<unknown> =>
      Promise.resolve({ frames: "not an array" });

    await expect(loadSpriteFrames("bunny", "walk", 50, "left")).rejects.toThrow(
      "frames must be string array"
    );
  });
});
...
describe("the real seams", () => {
  it("imports a module for real through the default importModule", async () => {
    ...
    _test_hooks.importModule = realImport;
    const module = (await realImport(
      "data:text/javascript,export const frames = ['x'];"
    )) as { frames: string[] };
