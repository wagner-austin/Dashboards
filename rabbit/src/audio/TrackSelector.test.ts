...
function createTestTrack(
  id: string,
  options?: { time?: "day" | "night" | "dawn" | "dusk"; location?: string }
): AudioTrack {
  return {
    id,
    path: `audio/${id}.mp3`,
...
describe("selectTrackByTime", () => {
  ...
  it("returns matching track for dawn", () => {
    const tracks: readonly AudioTrack[] = [createTestTrack("dawn-music", { time: "dawn" })];
    const result = selectTrackByTime(tracks, "dawn");
    expect(result?.id).toBe("dawn-music");
  });

  it("returns matching track for dusk", () => {
    const tracks: readonly AudioTrack[] = [createTestTrack("dusk-music", { time: "dusk" })];
  ...
  it("returns null when no matching track", () => {
    const tracks: readonly AudioTrack[] = [createTestTrack("day-music", { time: "day" })];
    const result = selectTrackByTime(tracks, "night");
...
describe("selectTrackByLocation", () => {
  ...
  it("returns null when no matching track", () => {
    const tracks: readonly AudioTrack[] = [createTestTrack("forest-music", { location: "forest" })];
...
describe("getDefaultTrack", () => {
  it("returns first track from array", () => {
    const tracks: readonly AudioTrack[] = [createTestTrack("first"), createTestTrack("second")];
  ...
  it("returns single track from single-element array", () => {
    const tracks: readonly AudioTrack[] = [createTestTrack("only")];
    const result = getDefaultTrack(tracks);
