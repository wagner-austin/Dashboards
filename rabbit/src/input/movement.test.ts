...
import type { AnimationState } from "../entities/Bunny.js";

const { processDepthMovement, processHorizontalMovement, resetCamera } = _test_hooks;

/** Pan speed used throughout these tests, in world units per second. */
const PAN_SPEED = 120;

/** Depth speed used throughout these tests, in world units per second. */
const DEPTH_SPEED = 30;
 ...
 */
function stateIn(animation: AnimationState, horizontal: "left" | "right" | null = null): InputState {
...
describe("processDepthMovement", () => {
  it("leaves the camera alone when not hopping", () => {
    ...
    processDepthMovement(state, 1, DEPTH_SPEED);
  ...
  it("moves into the scene when hopping away", () => {
    ...
    processDepthMovement(state, 1, DEPTH_SPEED);
    expect(state.camera.z).toBe(DEPTH_SPEED);
  });
  ...
  it("scales movement by the frame delta", () => {
    const state = stateIn({ kind: "hop", direction: "away", frameIdx: 0 });
    processDepthMovement(state, 0.5, DEPTH_SPEED);
    expect(state.camera.z).toBe(DEPTH_SPEED * 0.5);
  });
...
describe("processHorizontalMovement", () => {
  it.each(["walk", "jump"] as const)("runs faster during %s", (kind) => {
  ...
  it("scrolls right while walking right", () => {
    ...
    processHorizontalMovement(state, 1, PAN_SPEED);
    expect(state.camera.x).toBe(PAN_SPEED);
  });
  ...
  it("does not scroll during a transition", () => {
    const state = stateIn(
      { kind: "transition", type: "walk_to_idle", frameIdx: 0, pendingAction: null, returnTo: "idle" },
      "right"
    );
    processHorizontalMovement(state, 1, PAN_SPEED);
    expect(state.camera.x).toBe(0);
