...
import { _test_hooks, type InputArbiter } from "./arbiter.js";
import { NEUTRAL_INTENT, createIntent } from "./intent.js";
import type { InputState } from "./state.js";
import type { BunnyFrames, BunnyState, BunnyTimers } from "../entities/Bunny.js";
import {
  createTestBunnyState,
  createTestFrames,
  createTestInputState,
  createTestTimers,
...
describe("resolveIntent", () => {
  it("yields to the autopilot when the user asks for nothing", () => {
    expect(resolveIntent(NEUTRAL_INTENT, createIntent("left", null))).toStrictEqual(
      createIntent("left", null)
    );
  });

  it("prefers the user over the autopilot", () => {
    expect(resolveIntent(createIntent("right", null), createIntent("left", null))).toStrictEqual(
      createIntent("right", null)
    );
  });
  ...
  it("counts a vertical-only user intent as a claim on control", () => {
    expect(resolveIntent(createIntent(null, "up"), createIntent("left", null))).toStrictEqual(
      createIntent(null, "up")
    );
  });
});
...
describe("jumpBlocked", () => {
   ...
   */
  function depsFor(bunny: BunnyState): { state: InputState; frames: BunnyFrames; timers: BunnyTimers } {
  ...
  it("blocks a jump while hopping", () => {
    expect(
      jumpBlocked(depsFor(createTestBunnyState({ kind: "hop", direction: "away", frameIdx: 0 })))
    ).toBe(true);
  });
