...
describe("createInputState", () => {
  it("starts with a neutral intent", () => {
    const state = createInputState(
      ...
      { width: 80, height: 24, charW: 8, charH: 16 },
      { x: 5, z: 12 },
  ...
  it("retains the supplied camera and viewport", () => {
    ...
    const state = createInputState(
      createTestBunnyState({ kind: "walk", frameIdx: 1 }, true),
      { width: 80, height: 24, charW: 8, charH: 16 },
      { x: 5, z: 12 },
      bounds
    );
 ...
 */
function createTestStateWithIntent(horizontal: "left" | "right" | null): ReturnType<typeof createInputState> {
  const state = createInputState(
    createTestBunnyState({ kind: "idle", frameIdx: 0 }),
    { width: 80, height: 24, charW: 8, charH: 16 },
    { x: 0, z: 0 },
