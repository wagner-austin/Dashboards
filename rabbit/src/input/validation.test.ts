...
const { isRecord, requireBoolean, requireNumber, validateAutorunConfig, DEFAULT_AUTORUN_CONFIG } =
  _test_hooks;
...
describe("requireBoolean", () => {
  ...
  it("names the offending field when the type is wrong", () => {
    expect(() => requireBoolean("yes", "enabled", true)).toThrow(
      'autorun: "enabled" must be a boolean'
    );
  });
});
...
describe("requireNumber", () => {
  ...
  it("rejects non-numbers", () => {
    expect(() => requireNumber("3", "idleDelay", 5, 0, 10)).toThrow(
      'autorun: "idleDelay" must be a number between 0 and 10'
    );
  });

  it("rejects values below the minimum", () => {
    expect(() => requireNumber(-1, "idleDelay", 5, 0, 10)).toThrow(
      'autorun: "idleDelay" must be a number between 0 and 10'
    );
  });

  it("rejects values above the maximum", () => {
    expect(() => requireNumber(11, "idleDelay", 5, 0, 10)).toThrow(
      'autorun: "idleDelay" must be a number between 0 and 10'
    );
  });

  it("rejects non-finite values", () => {
    expect(() => requireNumber(Number.POSITIVE_INFINITY, "idleDelay", 5, 0, 10)).toThrow(
      'autorun: "idleDelay" must be a number between 0 and 10'
    );
    expect(() => requireNumber(Number.NaN, "idleDelay", 5, 0, 10)).toThrow(
      'autorun: "idleDelay" must be a number between 0 and 10'
    );
  });
});
...
describe("validateAutorunConfig", () => {
  ...
  it("rejects probabilities outside zero to one", () => {
    expect(() => validateAutorunConfig({ turnChance: 1.5 })).toThrow(
      'autorun: "turnChance" must be a number between 0 and 1'
    );
    expect(() => validateAutorunConfig({ hopChance: -0.1 })).toThrow(
      'autorun: "hopChance" must be a number between 0 and 1'
    );
    expect(() => validateAutorunConfig({ jumpChance: 2 })).toThrow(
      'autorun: "jumpChance" must be a number between 0 and 1'
    );
  });

  it("rejects an inverted leg range", () => {
    expect(() => validateAutorunConfig({ minLeg: 9, maxLeg: 2 })).toThrow(
      'autorun: "minLeg" must not exceed "maxLeg"'
    );
  });

  it("rejects an inverted pause range", () => {
    expect(() => validateAutorunConfig({ minPause: 9, maxPause: 2 })).toThrow(
      'autorun: "minPause" must not exceed "maxPause"'
    );
  });
  ...
  it("rejects a non-boolean enabled flag", () => {
    expect(() => validateAutorunConfig({ enabled: 1 })).toThrow(
      'autorun: "enabled" must be a boolean'
    );
  });
});
