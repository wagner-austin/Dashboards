...
function requireNumber(
  ...
  if (typeof value !== "number" || !Number.isFinite(value) || value < min || value > max) {
    throw new Error(
      `autorun: "${field}" must be a number between ${String(min)} and ${String(max)}`
    );
  }
  return value;
}
 ...
 */
export function validateAutorunConfig(config: unknown): AutorunConfig {
  if (config === undefined) {
