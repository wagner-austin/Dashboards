...
import { _test_hooks, type KeyboardDeps, type KeyboardKeys } from "./Keyboard.js";
...
import {
  ...
  createTestKeyboardSource,
  createTestTimers,
  type TestKeyboardSource,
...
describe("keyboard source", () => {
  ...
  describe("movement keys", () => {
    function timedKey(type: "keydown" | "keyup", key: string, time: number, keys: KeyboardKeys, repeat = false): void {
      const event = new KeyboardEvent(type, { key, repeat });
      Object.defineProperty(event, "timeStamp", { value: time });
      if (type === "keydown") handleKeyDown(event, keys, deps);
      else handleKeyUp(event, keys, deps);
    }
