...
// Game State
// ============================================================================
type BunnyAnimation = "walk" | "jump" | "idle" | "walk_to_idle" | "idle_to_walk";
...
interface GameState {
  ...
  // Ground scroll (separate from tree for stability during zoom)
  ...
  treeSizeTransitionProgress: number; // 0 to 1
  treeCenterX: number; // Center position for zoom anchoring
  currentSpeedMultiplier: number; // Lerped speed during transitions
  ...
  bunnyWalkToIdleFramesLeft: readonly string[];
  bunnyWalkToIdleFramesRight: readonly string[];
  treeSizes: { width: number; frames: readonly string[] }[];
}
...
async function init(): Promise<void> {
  ...
  let bunnyIdleFramesLeft: readonly string[] = [];
  let bunnyIdleFramesRight: readonly string[] = [];
  ...
  let bunnyWalkToIdleFramesLeft: readonly string[] = [];
  let bunnyWalkToIdleFramesRight: readonly string[] = [];
  ...
  const treeSizes: { width: number; frames: readonly string[] }[] = [];
  ...
  const treeTimer = createAnimationTimer(TREE_INTERVAL, () => {
    ...
    if (currentTreeSize === undefined) return;
  ...
  function render(currentTime: number): void {
    ...
    const transitionUpdate = updateSpeedTransition(
      state.treeSizeIdx,
      state.treeTargetSizeIdx,
      state.treeSizeTransitionProgress,
      deltaTime * 1000,
    ...
    const calcTreeY = (treeHeight: number, sizeIdx: number): number => {
