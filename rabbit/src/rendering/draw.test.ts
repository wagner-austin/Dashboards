...
function getCell(buffer: string[][], row: number, col: number): string {
  const r = buffer[row];
  if (r === undefined) throw new Error(`Row ${String(row)} not found`);
  const c = r[col];
  if (c === undefined) throw new Error(`Col ${String(col)} not found`);
  return c;
}

function setCell(buffer: string[][], row: number, col: number, value: string): void {
  const r = buffer[row];
  if (r === undefined) throw new Error(`Row ${String(row)} not found`);
  r[col] = value;
}
...
describe("drawSprite", () => {
  ...
  it("handles sparse buffer array gracefully", () => {
    ...
    sparseBuffer[0] = Array(10).fill(" ") as string[];
    sparseBuffer[4] = Array(10).fill(" ") as string[];
...
describe("drawSpriteFade", () => {
  ...
  it("handles sparse lines arrays gracefully", () => {
    ...
    const sparseOld: string[] = [];
    sparseOld.length = 2;
    sparseOld[0] = "A";
    // sparseOld[1] is undefined

    const sparseNew: string[] = [];
    sparseNew.length = 2;
