 ...
 */
export function resolveCharacterName(config: Config, override: string | null): string {
  if (override !== null) {
    return override;
  }
  const declared: unknown = config.character;
...
function resolveAnimation(
  ...
  if (width === undefined) {
    throw new Error(
      `character "${character}" animation "${required.name}" declares no widths`
    );
  }
  ...
  if (isDirectional !== required.directional) {
    ...
    throw new Error(
      `character "${character}" animation "${required.name}" must declare ${need}; ` +
...
export function resolveCharacterAnimations(
  ...
  const take = (name: string, directional: boolean): AnimationSource =>
    resolveAnimation(animations, { name, directional }, character);
