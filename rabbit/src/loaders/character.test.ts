...
function anim(directional: boolean, widths: readonly number[] = [40]): SpriteAnimationConfig {
  return {
...
function completeAnimations(): Record<string, SpriteAnimationConfig> {
  return {
    walk: anim(true, [50]),
    jump: anim(true, [50]),
    idle: anim(true),
...
describe("resolveCharacterName", () => {
  ...
  it("rejects a non-string character", () => {
    expect(() => resolveCharacterName(configWith({}, 7), null)).toThrow(
      'config: "character" must be a non-empty string'
    );
  });

  it("rejects an empty character", () => {
    expect(() => resolveCharacterName(configWith({}, "  "), null)).toThrow(
      'config: "character" must be a non-empty string'
    );
  });
});
...
describe("resolveCharacterAnimations", () => {
  ...
  it("rejects a character absent from config", () => {
    expect(() => resolveCharacterAnimations(configWith({}), "lion")).toThrow(
      'config.sprites has no character "lion"'
    );
  });

  it("rejects a sprite with no animations block", () => {
    const config = configWith({ tree1: { source: "t.gif", widths: [15] } });
    expect(() => resolveCharacterAnimations(config, "tree1")).toThrow(
      'config.sprites."tree1" declares no animations'
    );
  });
  ...
  it("names the animation that is missing", () => {
    ...
    expect(() => resolveCharacterAnimations(config, "lion")).toThrow(
      'character "lion" is missing animation "hop_toward"'
    );
  });
  ...
  it("rejects an animation declaring no widths", () => {
    ...
    expect(() => resolveCharacterAnimations(config, "lion")).toThrow(
      'character "lion" animation "walk" declares no widths'
    );
  });
  ...
  it("rejects a directional animation declared without directions", () => {
    ...
    expect(() => resolveCharacterAnimations(config, "lion")).toThrow(
      /animation "walk_to_turn_away" must declare two directions/
    );
  });
  ...
  it("rejects a hop declared with two directions", () => {
    ...
    expect(() => resolveCharacterAnimations(config, "lion")).toThrow(
      /animation "hop_away" must declare a single direction/
    );
  });
...
describe("resolveAnimation", () => {
  it("names the character in its errors", () => {
    expect(() =>
      resolveAnimation({}, { name: "walk", directional: true }, "lion")
    ).toThrow('character "lion" is missing animation "walk"');
  });
});
