 ...
 */
export function generateLayerWidths(config: LayerSpriteConfig): readonly number[] {
 ...
 */
export function layerToSizeIndex(
  currentLayer: number,
  defaultLayer: number,
  numSizes: number
): number | null {
 ...
 */
export function getVisibleLayerRange(
  defaultLayer: number,
  numSizes: number
): { readonly minLayer: number; readonly maxLayer: number } {
  const defaultSizeIdx = Math.max(0, numSizes - 1 - DEFAULT_SIZE_OFFSET);
 ...
 *     World Z coordinate.
 */
export function layerToWorldZ(layer: number): number {
 ...
 */
export function worldZToLayer(worldZ: number): number {
  return (worldZ - 50) / 5;
}
