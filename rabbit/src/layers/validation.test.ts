...
describe("requireLayerDefinition", () => {
  it("validates valid layer definition with all fields", () => {
    const result = requireLayerDefinition(
      { name: "test", type: "sprites", sprites: ["bunny"], parallax: 0.5 },
      0
    );
  ...
  it("throws for invalid parallax", () => {
    expect(() => requireLayerDefinition({ name: "test", parallax: "0.5" }, 0)).toThrow(
      'layers[0] "test": "parallax" must be a number'
    );
  });
...
describe("toValidatedLayer", () => {
  ...
  it("preserves explicit values", () => {
    const result = toValidatedLayer(
      { name: "sky", type: "static", parallax: 0.0, sprites: ["star"], tile: true },
      0
    );
