...
describe("requireLayerDefinition", () => {
  it("validates valid layer definition with all fields", () => {
    const result = requireLayerDefinition(
      { name: "test", type: "sprites", sprites: ["bunny"], positions: [-100, 0, 100], layer: 7 },
      0
    );
  ...
  it("throws for invalid type", () => {
    expect(() => requireLayerDefinition({ name: "test", type: "invalid" }, 0)).toThrow(
      'layers[0] "test": invalid "type" (must be static|tile|sprites)'
    );
  });

  it("throws for invalid sprites", () => {
    expect(() => requireLayerDefinition({ name: "test", sprites: "not-array" }, 0)).toThrow(
      'layers[0] "test": "sprites" must be string array'
    );
    expect(() => requireLayerDefinition({ name: "test", sprites: [1, 2, 3] }, 0)).toThrow(
      'layers[0] "test": "sprites" must be string array'
    );
  });

  it("throws for invalid positions", () => {
    expect(() => requireLayerDefinition({ name: "test", positions: "not-array" }, 0)).toThrow(
      'layers[0] "test": "positions" must be number array'
    );
    expect(() => requireLayerDefinition({ name: "test", positions: ["a", "b"] }, 0)).toThrow(
      'layers[0] "test": "positions" must be number array'
    );
  });

  it("throws for invalid layer", () => {
    expect(() => requireLayerDefinition({ name: "test", layer: "10" }, 0)).toThrow(
      'layers[0] "test": "layer" must be a number'
    );
  });

  it("throws for invalid tile", () => {
    expect(() => requireLayerDefinition({ name: "test", tile: "yes" }, 0)).toThrow(
      'layers[0] "test": "tile" must be a boolean'
    );
    expect(() => requireLayerDefinition({ name: "test", tile: 1 }, 0)).toThrow(
      'layers[0] "test": "tile" must be a boolean'
    );
  });
...
describe("toValidatedLayer", () => {
  ...
  it("preserves explicit values", () => {
    const result = toValidatedLayer(
      { name: "sky", type: "static", layer: 15, sprites: ["star"], positions: [-50, 50], tile: true },
      0
    );
...
describe("validateLayersConfig", () => {
  ...
  it("propagates validation errors from individual layers", () => {
    expect(() => validateLayersConfig([{ invalid: true }])).toThrow(
      'layers[0]: missing or invalid "name" field'
    );
  });
