...
import { createCompanion, stepCompanion, companionFrame, _test_hooks } from "./Companion.js";
import { createTestFrames } from "../testing/fixtures.js";
const STILL = { deltaTime: 0.1, cameraDx: 0, cameraDz: 0, facingRight: false, jumping: false };
describe("companion pursuit", () => {
    ...
    it("keeps running through the catch-up band instead of flickering between gaits", () => {
        const running = { ...createCompanion(), x: 165, mode: "run" };
    ...
    it("triggers one jump per leader jump and lands after its full duration", () => {
        ...
        let landed = airborne;
        for (let i = 0; i < 5; i++)
            landed = stepCompanion(landed, STILL);
