...
describe("chooseWalkDirection", () => {
    ...
    it("never turns at a zero turn chance", () => {
        const config = { ...BASE, turnChance: 0 };
...
describe("stepAutopilot", () => {
    ...
    it("counts down a pause without drawing", () => {
        const output = stepAutopilot({ kind: "pause", remaining: 2 }, input(0.5, 10), BASE, createSequenceRandom([]));
        expect(output.state).toStrictEqual({ kind: "pause", remaining: 1.5 });
    });
    it("starts a new leg when a pause expires", () => {
        const output = stepAutopilot({ kind: "pause", remaining: 0.25 }, input(0.5, 10), BASE, createSequenceRandom([0.25, 0.1, 0.4]));
    ...
    it("drops into a pause when a hop leg expires", () => {
        const output = stepAutopilot({ kind: "hop", direction: "down", remaining: 0.25 }, input(0.5, 10), BASE, createSequenceRandom([0.5]));
        expect(output.state).toStrictEqual({ kind: "pause", remaining: 2 });
    });
