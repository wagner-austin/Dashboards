...
describe("getSpriteWidths", () => {
    ...
    it("throws for unknown sprite", () => {
        const config = createTestConfig();
        expect(() => getSpriteWidths(config, "unknown")).toThrow('Sprite "unknown" not found in config');
    });
    it("throws for sprite without widths", () => {
        const config = createTestConfig();
        // Create sprite with no widths at any level
        config.sprites.empty = {};
        expect(() => getSpriteWidths(config, "empty")).toThrow('Sprite "empty" has no widths defined');
    });
    it("throws for sprite with empty animations", () => {
        const config = createTestConfig();
        config.sprites.noAnim = { animations: {} };
        expect(() => getSpriteWidths(config, "noAnim")).toThrow('Sprite "noAnim" has no widths defined');
    });
    it("throws for sprite with undefined animation value", () => {
        ...
        const animationsWithUndefined = { idle: undefined };
        config.sprites.badAnim = { animations: animationsWithUndefined };
        expect(() => getSpriteWidths(config, "badAnim")).toThrow('Sprite "badAnim" has no widths defined');
    });
});
describe("createLayerInstances", () => {
    ...
    it("throws for sprite not in registry", () => {
        ...
        expect(() => createLayerInstances(layers, registry, 100)).toThrow('Sprite "unknown" not found in registry');
    });
