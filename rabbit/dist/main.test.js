...
function testAnimation(directional, width) {
    return {
 ...
 */
function createTestRunProgressiveLoadFn(bunnyFrames) {
    ...
    it("starts the lion scene and advances perception before and after companion loading", async () => {
        ...
        const deps = {
            ...
            runProgressiveLoadFn: (_config, _character, _registry, _progress, loaded) => {
                ...
                if (first === undefined)
                    throw new Error("Expected queued render");
                first(1000);
                return Promise.resolve();
            },
