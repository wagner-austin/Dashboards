...
import { validateColorsConfig, DEFAULT_LAYER_COLORS, _test_hooks, } from "./colors.js";
...
describe("validateColorsConfig", () => {
    ...
    it("fills absent fields from the defaults, keeping the rest", () => {
        const result = validateColorsConfig({ actor: "#6db3ff" });
    ...
    it("rejects a non-string colour", () => {
        expect(() => validateColorsConfig({ actor: 255 })).toThrow('colors: "actor" must be a non-empty string');
    });
    it("rejects an empty colour", () => {
        expect(() => validateColorsConfig({ world: "" })).toThrow('colors: "world" must be a non-empty string');
    });
    it("rejects a whitespace-only colour", () => {
        ...
        expect(() => validateColorsConfig({ foreground: "   " })).toThrow('colors: "foreground" must be a non-empty string');
    });
});
describe("isRecord", () => {
...
describe("requireColor", () => {
    ...
    it("names the field it rejected", () => {
        expect(() => requireColor(42, "world", "#e0e0e0")).toThrow('colors: "world" must be a non-empty string');
    });
});
