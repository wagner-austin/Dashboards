 ...
 */
function getFadedChar(ch, visibility) {
    if (visibility >= 1)
        return ch;
    // Characters not in CHAR_DENSITY are treated as maximum density (0)
...
export function drawSprite(buffer, lines, x, y, width, height) {
 ...
 */
function sampleSprite(lines, spriteWidth, normX, // -0.5 to 0.5 (left to right)
normY // -1 to 0 (top to bottom)
) {
...
export function drawSpriteFade(buffer, oldLines, newLines, oldCenterX, oldCenterY, newCenterX, newCenterY, oldWidth, newWidth, width, height, progress, // 0 = all old, 1 = all new
