...
import { calcTreeY, getTreeFrame, getTreeTransitionFrames } from "../entities/Tree.js";
...
export function renderFrame(state, bunnyFrames, treeSizes, screen, currentTime, scrollSpeed, transitionDurationMs) {
    const deltaTime = state.lastTime > 0 ? (currentTime - state.lastTime) / 1000 : 0;
    // Update tree transition
    const transitionUpdate = updateSpeedTransition(state.treeState.sizeIdx, state.treeState.targetSizeIdx, state.treeState.sizeTransitionProgress, deltaTime * 1000, transitionDurationMs);
    ...
    if (state.bunnyState.isWalking && state.bunnyState.currentAnimation === "walk") {
        ...
        const scrollUpdate = calculateScrollUpdate(state.groundScrollX, state.treeState.centerX, scrollAmount, state.bunnyState.facingRight, width, 180);
