...
export function renderFrame(state, bunnyFrames, layers, currentTime) {
    ...
    if (state.adventure !== null) {
        ...
        const position = project(camera.x + visual.companion.x, camera.z + config.nearZ + Math.max(0, visual.companion.z), camera, width, height, config);
        if (position.visible) {
            ...
            drawSprite(companionBuffer, scaled, position.x - Math.floor(spriteWidth / 2), height - scaled.length - 2 + position.y - baseline.y, width, height);
        }
...
function scaleCharacter(lines, scale) {
    ...
    return Array.from({ length: targetHeight }, (_, row) => {
        ...
        if (source === undefined)
            throw new Error("RABBIT_SPRITE_EMPTY: Character canvas has no rows.");
