 ...
 */
export async function loadAdventureAssets(config, leader) {
    ...
    if (width === undefined)
        throw new Error("RABBIT_ALERT_CONFIG: Bunny alert animation needs a width.");
    const runWidth = config.sprites.lion?.animations?.run?.widths[0];
    if (runWidth === undefined)
        throw new Error("RABBIT_RUN_CONFIG: Lion run animation needs a width.");
...
//# sourceMappingURL=adventure-io.js.map