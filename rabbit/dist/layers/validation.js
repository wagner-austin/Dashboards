 ...
 */
function getSpriteAtIndex(sprites, index) {
    if (sprites.length === 0) {
...
export function generateAutoLayers(config) {
    const layers = [];
 ...
 */
export function requireAutoLayersConfig(value) {
    if (!isRecord(value)) {
 ...
 */
function requireLayerDefinition(value, index) {
    if (!isRecord(value)) {
    ...
    if (behavior !== undefined && !isBehaviorPreset(behavior)) {
        throw new Error(`layers[${String(index)}] "${name}": "behavior" must be static|background|midground|foreground`);
    }
 ...
 */
function toValidatedLayer(definition, zIndex) {
    return {
 ...
 */
export function processLayersConfig(layers, autoLayers) {
    const allLayers = [];
 ...
 */
export function validateLayersConfig(layers) {
    if (!Array.isArray(layers)) {
