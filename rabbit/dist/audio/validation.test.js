...
describe("requireAudioTrack", () => {
    it("validates valid track with all fields", () => {
        const result = requireAudioTrack({ id: "test", path: "audio/test.mp3", volume: 0.8, loop: true, tags: {} }, 0);
    ...
    it("validates track with time tag", () => {
        const result = requireAudioTrack({ id: "day-music", path: "audio/day.mp3", volume: 1.0, loop: true, tags: { time: "day" } }, 0);
        expect(result.tags.time).toBe("day");
    });
    it("validates track with location tag", () => {
        const result = requireAudioTrack({ id: "forest-music", path: "audio/forest.mp3", volume: 0.5, loop: false, tags: { location: "forest" } }, 0);
        expect(result.tags.location).toBe("forest");
    });
    it("validates track with both tags", () => {
        const result = requireAudioTrack({ id: "combo", path: "audio/combo.mp3", volume: 0.7, loop: true, tags: { time: "night", location: "cave" } }, 0);
    ...
    it("throws for missing path", () => {
        expect(() => requireAudioTrack({ id: "test" }, 0)).toThrow('audio.tracks[0] "test": missing or invalid "path" field');
        expect(() => requireAudioTrack({ id: "test", path: "" }, 0)).toThrow('audio.tracks[0] "test": missing or invalid "path" field');
        expect(() => requireAudioTrack({ id: "test", path: 123 }, 0)).toThrow('audio.tracks[0] "test": missing or invalid "path" field');
    });
    it("throws for invalid volume", () => {
        expect(() => requireAudioTrack({ id: "test", path: "a.mp3" }, 0)).toThrow('audio.tracks[0] "test": "volume" must be a number between 0 and 1');
        expect(() => requireAudioTrack({ id: "test", path: "a.mp3", volume: "0.5" }, 0)).toThrow('audio.tracks[0] "test": "volume" must be a number between 0 and 1');
        expect(() => requireAudioTrack({ id: "test", path: "a.mp3", volume: -0.1 }, 0)).toThrow('audio.tracks[0] "test": "volume" must be a number between 0 and 1');
        expect(() => requireAudioTrack({ id: "test", path: "a.mp3", volume: 1.1 }, 0)).toThrow('audio.tracks[0] "test": "volume" must be a number between 0 and 1');
    });
    it("throws for invalid loop", () => {
        expect(() => requireAudioTrack({ id: "test", path: "a.mp3", volume: 0.5 }, 0)).toThrow('audio.tracks[0] "test": "loop" must be a boolean');
        expect(() => requireAudioTrack({ id: "test", path: "a.mp3", volume: 0.5, loop: "yes" }, 0)).toThrow('audio.tracks[0] "test": "loop" must be a boolean');
        expect(() => requireAudioTrack({ id: "test", path: "a.mp3", volume: 0.5, loop: 1 }, 0)).toThrow('audio.tracks[0] "test": "loop" must be a boolean');
    });
    it("throws for invalid tags", () => {
        expect(() => requireAudioTrack({ id: "test", path: "a.mp3", volume: 0.5, loop: true }, 0)).toThrow('audio.tracks[0] "test": "tags" must be a valid TrackTags object');
        expect(() => requireAudioTrack({ id: "test", path: "a.mp3", volume: 0.5, loop: true, tags: null }, 0)).toThrow('audio.tracks[0] "test": "tags" must be a valid TrackTags object');
        expect(() => requireAudioTrack({ id: "test", path: "a.mp3", volume: 0.5, loop: true, tags: { time: "invalid" } }, 0)).toThrow('audio.tracks[0] "test": "tags" must be a valid TrackTags object');
    });
});
describe("validateAudioConfig", () => {
    ...
    it("throws for invalid masterVolume", () => {
        expect(() => validateAudioConfig({ enabled: true })).toThrow('audio: "masterVolume" must be a number between 0 and 1');
        expect(() => validateAudioConfig({ enabled: true, masterVolume: "0.5" })).toThrow('audio: "masterVolume" must be a number between 0 and 1');
        expect(() => validateAudioConfig({ enabled: true, masterVolume: -0.1 })).toThrow('audio: "masterVolume" must be a number between 0 and 1');
        expect(() => validateAudioConfig({ enabled: true, masterVolume: 1.1 })).toThrow('audio: "masterVolume" must be a number between 0 and 1');
    });
    it("throws for invalid tracks", () => {
        expect(() => validateAudioConfig({ enabled: true, masterVolume: 0.5 })).toThrow('audio: "tracks" must be an array');
        expect(() => validateAudioConfig({ enabled: true, masterVolume: 0.5, tracks: "not-array" })).toThrow('audio: "tracks" must be an array');
        expect(() => validateAudioConfig({ enabled: true, masterVolume: 0.5, tracks: {} })).toThrow('audio: "tracks" must be an array');
    });
    it("throws for duplicate track ids", () => {
        expect(() => validateAudioConfig({
            enabled: true,
            masterVolume: 0.5,
            tracks: [
