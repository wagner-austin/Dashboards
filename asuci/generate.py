...
<head>
    ...
    <style>
        /* THE SHARED PALETTE IS LINKED ABOVE, NOT RESTATED HERE.
           ...
           status colours and the two extra greys this dashboard uses.
           The page HTML had been cleaned of the duplicated palette once
           already; this generator had not, so the scheduled regeneration on
           2026-09-12 put every owned token straight back and turned
           `make check` red on main. Fixing the output without fixing the
           thing that writes it lasts exactly until the next run. */
        :root {{
            --gold-light: #fff9e6;
            --success: #16a34a;
